...
if [ $1 = "status" ]
   then
      $megacli -LDInfo -Lall -aALL -NoLog > /tmp/virtual_drive_status.txt
      echo "###############################################"
      $megacli -AdpPR -Info -aALL -NoLog >> /tmp/virtual_drive_status.txt
      echo "###############################################"
      $megacli -LDCC -ShowProg -LALL -aALL -NoLog >> /tmp/virtual_drive_status.txt
      cat /tmp/virtual_drive_status.txt | more
   exit
fi
...
# the drive "good." Basically, Unconfigured(bad) to Unconfigured(good). 
if [ $1 = "good" ]
   then
        echo "under construction"
...
if [ $1 = "logs" ]
   then
      $megacli -FwTermLog -Dsply -aALL -NoLog > /tmp/megacli_log.txt
      cat /tmp/megacli_log.txt | more
   exit
fi
...
# script will send email to the address in the $EMAIL variable. 
# Add this method to a cron job to be run every few hours to notify of any
# issues.
if [ $1 = "checkNemail" ]
   then
      EMAIL="exabennett@gmail.com"
...
if [ $1 = "allinfo" ]
   then
      $megacli -AdpAllInfo -aAll -NoLog > /tmp/all_LSIcard_info.txt
      cat /tmp/all_LSIcard_info.txt | more
   exit
fi
...
# These are the defaults we like to use at Crawford.
# When setting up a new machine simply execute the "setdefaults"
...
if [ $1 = "setdefaults" ]
   then
      ...
      # $megacli -AdpSetProp -BootWithPinnedCache -1 -a0 -NoLog
      ...
      ## RAID rebuild rate to 80% (build quick before another failure)
      # $megacli -AdpSetProp \{RebuildRate -80\} -aALL -NoLog
      ## RAID check consistency rate to 80% (fast parity checks)
      # $megacli -AdpSetProp \{CCRate -80\} -aALL -NoLog
